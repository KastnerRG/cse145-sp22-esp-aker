...
#define DRV_NAME	"dummy_rtl"

/* <<--regs-->> */
#define DUMMY_SIZE_REG 0x40

struct dummy_rtl_device {
...
static inline struct dummy_rtl_device *to_dummy(struct esp_device *esp)
{
	return container_of(esp, struct dummy_rtl_device, esp);
}

static void dummy_prep_xfer(struct esp_device *esp, void *arg)
{
	struct dummy_rtl_access *a = arg;

	/* <<--regs-config-->> */
	iowrite32be(a->size, esp->iomem + DUMMY_SIZE_REG);
...
{
	/* struct dummy_rtl_device *dummy = to_dummy(esp); */
	/* struct dummy_rtl_access *a = arg; */
...
static struct esp_driver dummy_driver = {
	...
	.ioctl_cm	= DUMMY_RTL_IOC_ACCESS,
	.arg_size	= sizeof(struct dummy_rtl_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("dummy_rtl driver");
